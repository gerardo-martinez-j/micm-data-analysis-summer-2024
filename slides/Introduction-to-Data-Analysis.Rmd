...
We will introduce the package `tidyverse`. `tidyverse` is a collection of several packages with an underlying syntax and philosophy on how to build and read R code --- you can find more information about it [here](https://www.tidyverse.org). We will be using two of its packages: `dplyr` and `ggplot2`. Let us start by loading `tidyverse`.
...
*Exercise*. Transform all the entries of `social_media` that have location equal to "UK" from "UK" to "United Kingdom" using `mutate()`.
...
Let us load the file `crab_data.csv`. This data set includes 3893 entries corresponding to different measures from crabs; the data set is a subset of the data set found [here](https://www.kaggle.com/datasets/sidhus/crab-age-prediction). Let us say we want to build a scatterplot to see if there is a linear association between `Length` and `Height`. The basic functionality is to call the function `ggplot` in the following way
```{r eval = FALSE}
